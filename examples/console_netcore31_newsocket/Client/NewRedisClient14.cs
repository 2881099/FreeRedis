...
namespace console_netcore31_newsocket
{
    ...
    public class NewRedisClient14 : RedisClientBase
    {
        ...
        protected internal override void Handler(in ReadOnlySequence<byte> sequence)
        {
            ...
            //Task.Run(() => { HandlerResult(tempParameters, tempTail); });
            ThreadPool.QueueUserWorkItem(new WaitCallback((state) => { HandlerResult(tempParameters, tempTail); }));
            void HandlerResult(SingleLinkNode2<bool> head, SingleLinkNode2<bool> tail)
            {
