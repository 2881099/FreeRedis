...
/// </remarks>
internal class SourceConcurrentQueue<T> : IProducerConsumerCollection<T>
{
    ...
    /// </summary>
    public SourceConcurrentQueue(PipeWriter sender)
    {
        _sender = sender;
    ...
    /// null.</exception>
    public SourceConcurrentQueue(IEnumerable<T> collection)
    {
        if (collection == null)
        {
    ...
    object ICollection.SyncRoot
    {
        get
        {
            throw new NotSupportedException("不支持同步！");
        }
    }
    ...
    private class Segment
    {
        ...
        private volatile SourceConcurrentQueue<T> _source;
