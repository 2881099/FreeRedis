...
/// </remarks>
public class SourceConcurrentQueue2<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
{
    ...
    object ICollection.SyncRoot { get { throw new Exception("Not supported!"); return default; } }
        ...
        while (true)
        {
            ...
            lock (_crossSegmentLock)
            {
                if (tail == _tail)
                {
                    ...
                    var newTail = new ConcurrentQueueSegment<T>(_sender,nextSize);
    ...
    public void Clear()
    {
        lock (_crossSegmentLock)
        {
            ...
            _tail = _head = new ConcurrentQueueSegment<T>(_sender, InitialSegmentLength);
        }
    }
}
