...
namespace FreeRedis
{
	partial class RedisClient
	{
		...
		public Dictionary<string, string> CfInfo(string key) => Call<string[], Dictionary<string, string>>("CF.INFO".Input(key).FlagKey(key), rt => rt.NewValue(a => a.MapToHash<string>(Encoding)).ThrowOrValue());
    }
}
